	...
	<div class="container">
		...
		<div class="panel panel-default">
			<table class="table table-borderd">
				...
				<tbody>
					<c:forEach items="${dto.cities}" var="cities">
						<spring:url value="country/expand/${cities.id}" var="expandUrl" />
						<spring:url value="${baseUrl}/edit/${cities.id}" var="editUrl" />
						<spring:url value="${baseUrl}/delete${cities.id}" var="deleteUrl" />


						<tr>
							<td><a href="${expandUrl}">${cities.id}</a></td>
							<td>${cities.zip}</td>
							<td>${cities.name}</td>
